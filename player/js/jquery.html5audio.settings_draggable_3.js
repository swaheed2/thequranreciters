...
var hap_player1, hap_players = [hap_player1], drag_to_play_thumb;
jQuery(document).ready(function($) {
	...
	$('#wrapper').css('zIndex',100);//thumbnails above player
	
	//make draggable from playlist selector into player playlist
	$(".hap_draggable").draggable({
		revert: "invalid",
	...
	//do some action on items dropped into player playlist 
	$("#componentWrapper").droppable({
		tolerance: "touch",
	 	drop: function(event, ui) {
		   ...
		   if(_item.hasClass('hap_play')){
			  //console.log('hap_play', _item); 
			  
			  //place this inside audioPlayerSetupDone callback (and modify accordingly) if tracks for whatever reason are loaded faster then component inits itself because api_addHiddenTrack will not work otherwise!
			  var track, mp3, ogg='', title;
			  if(_item.attr('data-mp3') != undefined) mp3 = _item.attr('data-mp3');
			  if(mp3){
				  if(drag_to_play_thumb)drag_to_play_thumb.find('p[class=hap_title]').css({color:"#fff"});//set active text color 
				  _item.find('p[class=hap_title]').css({color:"#ff0"});
				  drag_to_play_thumb = _item;
				   
				  if(_item.attr('data-ogg') != undefined) ogg = _item.attr('data-ogg');
				  if(_item.attr('data-title') != undefined) title = _item.attr('data-title');
				  track = {mp3: mp3, ogg: ogg, title: title};
				  
				  api_inputAudio(hap_players[0],track);
		       }
		   }
	   },
